...
const getRegionCookie = function(){
	...
	const RegionAreaLayer = {
        ...
        methods : {
			...
			msg: function(code){
				return regionMessage[this.language][code]();
			}
        },
        template : `
        <Teleport to="body">
	...
	const regionApp = {
		...
		data: function() {
			return {
				...
				currentRegion: this.globalData.currentRegion,
		...
		methods: {
			...
			changeLink : function(){
                ...
                document.querySelectorAll(".region-link").forEach(it => {
					...
					href = href.replace(/([^.]+)(\.xbluesalt)/, me.currentRegion + '$2');
                    it.setAttribute('href', href);
				});

				let now = new Date();
				let expires = now.valueOf() + (1000 * 60 * 60 * 24 * 30);
				document.cookie = COOKIE_NAME + "=" + me.currentRegion + ";path=/;Expires=" + new Date(expires) + ";domain=." + this.globalData.domains.cookieDomain;

				if (this.globalData.currentRegion != me.currentRegion) {
					let region = "https://bslt." + me.currentRegion;
					let domain = this.globalData.domains.cookieDomain;
					let path = location.pathname;
					location.href = `${region}.${domain}${path}`;
				}
			}
		},
		computed : {
